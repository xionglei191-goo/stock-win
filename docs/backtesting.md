...
Python 是主回测引擎，确保策略逻辑、执行限制和研究数据库使用同一套实现。`combined` 运行 `chan_v1 + course49_v2`；各 `course49_v*_compare` 组合让版本共享快照；`adaptive_multi_strategy` 增加 40% 配对套利资金分舱。V4-V11 未完成样本外验证前不替换 `combined` 的默认成员。
...
回测结果写入 `backtests`、`backtest_equity`、`backtest_trades` 和 `backtest_states`。页面展示收益指标、权益曲线、逐笔交易、持仓变化、V1/V2 对比、市场风格时间线、风格/模式/退出归因、平均资金投入及实际股票池分布。
...
默认使用 8 个本地处理线程、24GB 内存 LRU 和 50GB 磁盘上限。行情批量起始为 800 只、专业事件为 500 只，空结果自动减半到 100 只；通达信请求永远单通道。本地服务最多同时运行三个作业，快照回放可以真正并行，冷任务只在取得 TQ 通道前等待。Course49 市场、资格、候选和版本特征按快照及特征版本缓存，对比实验只计算一次。
...
缓存快照先以 `BUILDING` 状态写入，所有数据和策略执行成功后才原子发布为 `READY`。失败项不会命中；治理顺序为失败项、可重建特征、未引用数据。被回测或运行引用的证据快照不自动删除。
