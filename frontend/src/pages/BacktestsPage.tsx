...
const fallbackStrategyNames: Record<string, string> = {
  ...
  pairs_arbitrage_v1: '配对套利 V1',
  adaptive_multi_strategy: '自适应多策略组合',
}
...
export default function BacktestsPage() {
  ...
  function submit(event: FormEvent) {
    ...
    if (form.universe === 'custom' && stockCodes.length === 0) {
    ...
    run.mutate({
      ...
      max_stocks: form.maxStocks ? Number(form.maxStocks) : undefined,
  ...
  function replayWithDoubleCosts() {
    if (!detail.data || !canReplayCourse49) return
    replay.mutate({
  ...
  return <>
    ...
    <section className="backtest-config panel">
      ...
      <form className="config-grid" onSubmit={submit}>
        ...
        <label className="field"><span>股票池</span><select value={form.universe} onChange={(event) => update('universe', event.target.value as BacktestUniverse)}>
          {Object.entries(universeNames).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
        </select></label>
        ...
        <label className="field"><span>运行范围</span><select value={form.samplingMode} onChange={(event) => update('samplingMode', event.target.value as SamplingMode)}>
          <option value="full">全量</option><option value="stratified">分层样本</option>
        </select></label>
        {form.samplingMode === 'stratified' && <label className="field"><span>股票上限</span><input type="number" min="1" value={form.maxStocks} onChange={(event) => update('maxStocks', event.target.value)} /></label>}
    ...
    {detail.data?.parameters && <section className="pool-summary panel">
      <div className="section-heading"><h2>实际股票池</h2><div><span>{detail.data.parameters.sampling_mode === 'stratified' ? `分层样本 · 种子 ${detail.data.parameters.sample_seed}` : '全量运行'}</span>{canReplayCourse49 && <button className="button button--secondary" type="button" disabled={activeJob} onClick={replayWithDoubleCosts}><FlaskConical size={16} />2 倍成本回放</button>}</div></div>
